...
export default function HeroBackground() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      grad.addColorStop(0, "rgba(79, 140, 255, 0.04)");
      ...
      for (const star of stars) {
        ...
        // Pulsing alpha
        const pulse = Math.sin(frameCount * star.speed * 0.02) * 0.15 + star.alpha;
        const isAccent = star.radius > 1.4;

        // Glow halo for brighter nodes
        if (isAccent && pulse > 0.4) {
          ctx.beginPath();
          ctx.arc(star.x, star.y, star.radius * 3, 0, Math.PI * 2);
          ctx.fillStyle = `rgba(79, 140, 255, ${pulse * 0.08})`;
          ctx.fill();
        }
        ...
        ctx.fillStyle = isAccent
          ? `rgba(79, 140, 255, ${pulse})`
          : `rgba(156, 163, 175, ${pulse * 0.7})`;
        ctx.fill();
      }
      ...
      for (const conn of connectionsRef.current) {
        ...
        ctx.strokeStyle = `rgba(79, 140, 255, ${conn.alpha * 0.8})`;
        ctx.lineWidth = 0.4;
        ctx.stroke();
      }
